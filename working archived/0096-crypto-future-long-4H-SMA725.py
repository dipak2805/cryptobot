...
def check_conditions(symbol):
    ...
    # Check SMA cross on 4H
    if not (prev_sma7 < prev_sma25 and curr_sma7 > curr_sma25):
        print(f"❌ {symbol} - No bullish SMA cross on 4H")
        return None
    ...
    if not (curr_sma7_1h > curr_sma25_1h):
        print(f"❌ {symbol} - SMA7 not above SMA25 on 1H")
        return None
...
def run_strategy():
    print("🔍 Starting SMA Cross Screener...\n")
    ...
    for symbol in target_symbols:
        ...
        try:
            ...
            if result:
                send_telegram_message(f"✅ SMA Cross Signal: {result['symbol']} | Price: {result['price']}")
    ...
    with open("future_usdt_sma_cross_signal.txt", "w") as f:
