...
def check_conditions(symbol):
    ...
    # Bullish cross check on 1H
    if not (prev_sma7 < prev_sma25 and curr_sma7 > curr_sma25):
        print(f"❌ {symbol} - No bullish SMA cross on 1H")
        return None

    # --- 15m data (secondary check) ---
    ...
    if not (curr_sma7_15m > curr_sma25_15m):
        print(f"❌ {symbol} - SMA7 not above SMA25 on 15m")
        return None
...
def run_strategy():
    print("🔍 Starting 1H + 15m SMA Cross Screener...\n")
    ...
    for symbol in target_symbols:
        ...
        try:
            ...
            if result:
                send_telegram_message(f"✅ 1H+15m Long Signal: {result['symbol']} | Price: {result['price']}")
    ...
    with open("future_usdt_1h15m_long_signal.txt", "w") as f:
    ...
    summary = (
        f"✅ 1H + 15m SMA Cross Scan Complete\n"
